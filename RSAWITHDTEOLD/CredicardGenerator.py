...
'''def create(number):
    with open('creditcard.csv', 'w') as outcsv:

        # configure writer to write standard csv file
        writer = csv.writer(outcsv, lineterminator='\n')
        # Write item to outcsv
        writer.writerow([number, random.getrandbits(32)])'''
...
def credit_card_number(rnd, prefixList, length, howMany):
    result = []
    count=0
    outcsv=open('creditcard.csv', 'a')
    while count< howMany:
        ccnumber = copy.copy(rnd.choice(prefixList))

        # result.append(completed_number(ccnumber, length))
        #print(completed_number(ccnumber, length))
        count+=1
        writer = csv.writer(outcsv, lineterminator='\n')
        # Write item to outcsv
        writer.writerow([completed_number(ccnumber, length), random.getrandbits(32)])

        #create(completed_number(ccnumber, length))
...
mastercard = credit_card_number(generator, mastercardPrefixList, 16, 1000)
