import Tkinter
import base64
from RSAonCredicardNo import decrypt
from RSAonCredicardNo import encrypt
import tkMessageBox
...
class crypt_mod(Tkinter.Tk):
    ...
    def initialize(self):
        ...
        plain_label = Tkinter.Label(text='Enter The CredicardNo:')
    ...
    def encrypt(self):
        if not self.ptxt.get():
            ...
            return
       	'''if not (len(isbn) == 16 and isbn.isdigit()):
    		tkMessageBox.showinfo(message="number must be of 16 characters long")
            	return  '''
